...
<script>
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            session.set({ user_id: response.user_id, username: response.username, token: response.token, email_verified: response.email_verified });
            goto('/');
        } catch (error) {
                ...
                <form on:submit|preventDefault={handleSubmit} class="card-body">
                    ...
                    {#if emailNotVerified}
                        <div class="alert alert-warning mt-4">
                            <div>
                                <span class="label-text">Your email is not verified. Please check your email for the verification link.</span>
                            </div>
                        </div>
                    {/if}
                </form>
